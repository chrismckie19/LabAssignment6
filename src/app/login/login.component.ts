...
export class LoginComponent implements OnInit {
  ...
  login(user: IUser) {
    const defaultUser: IUser = { username: 'chrismckie', password: 'chris123' }
    if (user.username !== '' && user.password !== '') {
      if (user.username === defaultUser.username && user.password === defaultUser.password) {
        ...
        this.toastService.showToast('danger', 'Login failed! Please check your username or password!', 15000);
            ...
            // show error toast to user
            this.toastService.showToast('danger', 'Login failed! Please specify your username and password!', 15000,);
    }
  }
